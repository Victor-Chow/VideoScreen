...
/**
 * Manages screenshot capture, OCR time recognition, watermark rendering, and saving.
 */
class ScreenshotManager(private val context: Context) {
    ...
    data class CapturedFrame(
        ...
        val ocrDate: Date?       // Time recognized from video frame via OCR
    )

    private val captures = mutableListOf<CapturedFrame>()
    val ocrRecognizer = OcrTimeRecognizer()

    /** Whether to use OCR for timestamp instead of EXIF. */
    var useOcrTime: Boolean = true

    val captureCount: Int get() = captures.size

    /**
     * Capture a frame at the given position from the video file path.
     */
    fun captureFrame(
        videoPath: String?,
        positionMs: Long
        ...
        return try {
            ...
            if (bitmap != null) {
                val ocrDate = if (useOcrTime) ocrRecognizer.recognizeSync(bitmap) else null
                captures.add(CapturedFrame(bitmap, positionMs, ocrDate))
            }
            bitmap
        } catch (e: Exception) {
            null
    ...
    /**
     * Capture frame using file descriptor.
     */
     ...
     */
    fun applyWatermark(source: Bitmap, position: WatermarkPosition, frame: CapturedFrame): Bitmap {
        if (position == WatermarkPosition.NONE) return source

        val result = source.copy(Bitmap.Config.ARGB_8888, true)
        val canvas = Canvas(result)

        val watermarkText = buildWatermarkText(frame)
    ...
    /**
     * Build watermark text from the captured frame.
     * Priority: OCR time > elapsed time
     */
    private fun buildWatermarkText(frame: CapturedFrame): String {
        return if (frame.ocrDate != null) {
            val sdf = SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault())
            sdf.format(frame.ocrDate)
        } else {
            // Fallback: elapsed time from video start
    ...
    fun saveAll(watermarkPosition: WatermarkPosition): Int {
        ...
        for ((index, frame) in captures.withIndex()) {
            try {
                val watermarked = applyWatermark(frame.bitmap, watermarkPosition, frame)
